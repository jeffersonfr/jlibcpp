...
//  Return a number distributed following a BreitWigner function with mean and gamma
   ...
   double rval, displ;
   rval = 2 * RandomDouble() - 1;
   displ = 0.5*gamma*Math<double>::Tan(rval*Math<double>::PI/2.0);

   return (mean+displ);
}

void Random::Circle(double *x, double *y, double r)
{
   // generates random vectors, uniformly distributed over a circle of given radius.
   //   Input : r = circle radius
   //   Output: x,y a random 2-d vector of length r

   double phi = Uniform(0, jmath::Math<double>::PI*2.0);
   (*x) = r*Math<double>::Cos(phi);
   (*y) = r*Math<double>::Sin(phi);
}

double Random::Exp(double tau)
{
...
//          exp( -t/tau )
   ...
   double t = -tau * Math<double>::Log( x ); // convert to exponential distribution
   return t;
}
...
//      Return a number distributed following a gaussian with mean and sigma
   ...
   result = mean + sigma*Math<double>::Sin(x)*Math<double>::Sqrt(-2*Math<double>::Log(y));
   return result;
}
...
double Random::Landau(double mpv, double sigma)
{
...
//  Converted by Rene Brun from CERNLIB routine ranlan(G110)
   ...
   if (i >= 70 && i < 800) {
      ...
      ranlan = ((0.99858950+(3.45213058E1+1.70854528E1*u)*u)/
                ...
                (-Math<double>::Log(-0.91893853-v)-1);
   } else {
      u = 1-x;
      v = u*u;
      if (x <= 0.999) {
...
//
   ...
   // for large value we use inversion method
      ...
      g = mean*alxm - Math<double>::LnGamma(mean + 1.0);

      do {
         do {
            y = Math<double>::Tan(pi*RandomDouble());
            em = sq*y + mean;
         } while( em < 0.0 );

         em = Math<double>::Floor(em);
...
double Random::PoissonDouble(double mean)
{
...
//
   ...
   // for large value we use inversion method
      ...
      // use Gaussian approximation vor very large values
      return Gaussian(0,1)*Math<double>::Sqrt(mean) + mean +0.5;
   }
}

void Random::Rannor(float &a, float &b)
{
//      Return 2 numbers distributed following a gaussian with mean=0 and sigma=1
   ...
   z = RandomDouble();
   x = z * 6.28318530717958623;
   r = Math<double>::Sqrt(-2*Math<double>::Log(y));
   a = (float)(r * Math<double>::Sin(x));
   b = (float)(r * Math<double>::Cos(x));
}

void Random::Rannor(double &a, double &b)
{
//      Return 2 numbers distributed following a gaussian with mean=0 and sigma=1
   ...
   z = RandomDouble();
   x = z * 6.28318530717958623;
   r = Math<double>::Sqrt(-2.0*Math<double>::Log(y));
   a = r * Math<double>::Sin(x);
...
void Random::Sphere(double *x, double *y, double *z, double r)
{
   ...
   double scale = 8.0 * r * Math<double>::Sqrt(0.25 - r2);
   (*x) = a*scale;
   (*y) = b*scale;
}

double Random::Uniform(double x1)
{
// returns a uniform deviate on the interval  ]0, x1].

   double ans = RandomDouble();
   return x1*ans;
}

double Random::Uniform(double x1, double x2)
{
