...
{
   double ax = Math<T>::Abs(x), ay = Math<T>::Abs(y);
   double amax, amin;
...
// math functions are defined so we have to include them here
#   include <math.h>
...
{
   	return (int64_t) (hypot((double)x, (double)y) + 0.5);
}
...
template<class T>int64_t Math<T>::NextPrime(int64_t x)
{
	...
	int64_t sqr = (int64_t) sqrt((double)x) + 1;
...
template<class T>double Math<T>::ErfInverse(double x)
{
   ...
   if(Math<T>::Abs(x) <= kEps) return kConst*x;
   ...
   if(Math<T>::Abs(x) < 1.0) {
      ...
      for (int iter=0; iter<kMaxit; iter++) {
         ...
         dy1 = Math<T>::Abs(x) - y1;
         if (Math<T>::Abs(dy1) < kEps)  {if (x < 0) return -erfi; else return erfi;}
         dy0    = y1 - y0;
         derfi *= dy1/dy0;
         y0     = y1;
         erfi  += derfi;
         if(Math<T>::Abs(derfi/erfi) < kEps) {if (x < 0) return -erfi; else return erfi;}
      }
   }
...
template<class T>double Math<T>::GamSer(double a,double x)
{
   ...
   for (int n=1; n<=itmax; n++) {
      ...
      del  = del*x/ap;
      sum += del;
      if (Math<T>::Abs(del) < Abs(sum*eps)) break;
...
template<class T>double Math<T>::Landau(double x, double mpv, double sigma, bool norm)
{
   ...
   if (v < -5.5) {
      ...
      u   = Math<T>::Exp(-v-1);
      den = Math<T>::Exp(-u)*Math<T>::Sqrt(u)*
...
template<class T>double Math<T>::PoissonI(double x, double par)
{
  ...
  // This is a non-smooth function
   ...
   if(x<0) return 0;
   if(x<1) return Math<T>::Exp(-par);
   double gam;
   int ix = int(x);
   if(x < kMaxInt) gam = Math<T>::Power(par,ix)/Math<T>::Gamma(ix+1);
   else            gam = Math<T>::Power(par,x)/Math<T>::Gamma(x+1);
      ...
      for (int i=0; i<nprob; i++){
         ...
         if (npm < 1){
            ...
            if (npm - j -1 > 1e-14){
               ...
               if (isSorted) xj = x[j];
               else xj = Math<T>::KOrdStat(n, x, j, ind);
               if (type==1) quantiles[i] = xj;
               if (type==2) {
                  if (isSorted) xjj = x[j+1];
                  else xjj = Math<T>::KOrdStat(n, x, j+1, ind);
...
template<class T>double Math<T>::BesselK1(double x)
{
   ...
   if (x <= 2) {
      y = x*x/4;
      result = (log(x/2.)*Math<T>::BesselI1(x))+(1./x)*(p1+y*(p2+y*(p3+y*(p4+y*(p5+y*(p6+y*p7))))));
   } else {
      y = 2/x;
      result = (exp(-x)/sqrt(x))*(q1+y*(q2+y*(q3+y*(q4+y*(q5+y*(q6+y*q7))))));
   }
...
template<class T>double Math<T>::BesselK(int n,double x)
{
   ...
   if (n==0) return Math<T>::BesselK0(x);
   if (n==1) return Math<T>::BesselK1(x);
...
template<class T>double Math<T>::BesselI(int n,double x)
{
   ...
   if (n==0) return Math<T>::BesselI0(x);
   if (n==1) return Math<T>::BesselI1(x);

   if (x == 0) return 0;
   if (Math<T>::Abs(x) > kBigPositive) return 0;
...
template<class T>double Math<T>::StruveL0(double x)
{
   ...
   if (x<=20.) {
      ...
      for (int i=1; i<=60;i++) {
         r*=(x/(2*i+1))*(x/(2*i+1));
         s+=r;
         if(Math<T>::Abs(r/s)<1.e-12) break;
      }
      sl0=a0*s;
   } else {
      km=int(5*(x+1.0));
      ...
      for (i=1; i<=km; i++) {
         r*=(2*i-1)*(2*i-1)/x/x;
         s+=r;
         if(Math<T>::Abs(r/s)<1.0e-12) break;
      }
      ...
      for (i=1; i<=16; i++) {
         ...
         bi0+=r;
         if(Math<T>::Abs(r/bi0)<1.0e-12) break;
      }
...
template<class T>double Math<T>::StruveL1(double x)
{
   ...
   if (x<=20.) {
      ...
      for (i=1; i<=60;i++) {
         r*=x*x/(4.0*i*i-1.0);
         s+=r;
         if(Math<T>::Abs(r)<Math<T>::Abs(s)*1.e-12) break;
      }
      sl1=2.0/pi*s;
   } else {
      s=1.0;
      ...
      for (i=1; i<=16; i++) {
         ...
         bi1+=r;
         if(Math<T>::Abs(r/bi1)<1.0e-12) break;
      }
      sl1+=a1*bi1;
   }
   return sl1;
...
template<class T>double Math<T>::LaplaceDist(double x, double alpha, double beta)
{
   ...
   double temp;
   temp  = Math<T>::Exp(-Math<T>::Abs((x-alpha)/beta));
   temp /= (2.*beta);
   return temp;
}
...
template<class T>double Math<T>::StudentQuantile(double p, double ndf, bool lower_tail)
{
   ...
   if ((ndf-2)<1e-8) {
      quantile = Math<T>::Sqrt(2./(q*(2-q))-2);
   } else {
      if ((ndf-1)<1e-8) {
...
template<class T>double Math<T>::LandauI(double x)
{
   ...
   if (v < -5.5) {
      ...
      u = Math<T>::Exp(-v-1);
      lan = (Math<T>::Exp(-u)/Math<T>::Sqrt(u))*(p1[0]+(p1[1]+(p1[2]+(p1[3]+p1[4]*v)*v)*v)*v)/
          (q1[0]+(q1[1]+(q1[2]+(q1[3]+q1[4]*v)*v)*v)*v);
   }
   else if (v < 1)
...
template<class T>void Math<T>::VavilovSet(double rkappa, double beta2, bool mode, double *WCM, double *AC, double *HC, int &itype, int &npt)
{
   ...
   if (itype == 3) {
      ...
      p2 = AC[7]*AC[7];
      AC[11] = p2*(AC[1]*Math<T>::Exp(-AC[2]*(AC[7]+AC[5]*p2)-
                                    AC[3]*Math<T>::Exp(-AC[4]*(AC[7]+AC[6]*p2)))-0.045*y/AC[7])/(1+x*y*AC[7]);
