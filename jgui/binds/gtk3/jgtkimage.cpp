...
GTKImage::GTKImage(jpixelformat_t pixelformat, int width, int height):
	jgui::Image(pixelformat, width, height)
{
	jcommon::Object::SetClassName("jgui::GTKImage");
	...
	_graphics = new GTKGraphics(NULL, pixelformat, width, height);

	dynamic_cast<GTKHandler *>(GFXHandler::GetInstance())->Add(this);
}

GTKImage::GTKImage(std::string file):
	jgui::Image(JPF_ARGB, -1, -1)
{
	...
	// _graphics = new GTKGraphics(NULL, JPF_ARGB, desc.width, desc.height);
...
jsize_t GTKImage::GetImageSize(std::string img)
{
	jsize_t t;
...
Image * GTKImage::CreateImageStream(jio::InputStream *stream)
{
	if ((void *)stream == NULL) {
		return NULL;
	}
...
void GTKImage::Release()
{
}

void GTKImage::Restore()
{
}

Image * GTKImage::Flip(Image *img, jflip_flags_t mode)
{
	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(img->GetGraphics())->_cairo_context);
	...
	GTKImage *image = new GTKImage(img->GetPixelFormat(), width, height);
	cairo_t *cairo_context = dynamic_cast<GTKGraphics *>(image->GetGraphics())->_cairo_context;
...
Image * GTKImage::Rotate(Image *img, double radians, bool resize)
{
	cairo_t *src_context = dynamic_cast<GTKGraphics *>(img->GetGraphics())->_cairo_context;
	...
	if (resize == true) {
		...
		GTKImage *dst = new GTKImage(img->GetPixelFormat(), iw, ih);
		cairo_t *dst_context = dynamic_cast<GTKGraphics *>(dst->GetGraphics())->_cairo_context;
...
Image * GTKImage::Scale(Image *img, int width, int height)
{
	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(img->GetGraphics())->_cairo_context);
...
Image * GTKImage::Crop(Image *img, int x, int y, int width, int height)
{
	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(img->GetGraphics())->_cairo_context);
...
Image * GTKImage::Blend(Image *img, double alpha)
{
	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(img->GetGraphics())->_cairo_context);
	...
	GTKImage *image = new GTKImage(img->GetPixelFormat(), img->GetWidth(), img->GetHeight());
	cairo_t *cairo_context = dynamic_cast<GTKGraphics *>(image->GetGraphics())->_cairo_context;
...
Image * GTKImage::Colorize(Image *img, Color color)
{
	GTKImage *image = (GTKImage *)Blend(img, 1.0);

	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(image->GetGraphics())->_cairo_context);
...
void GTKImage::SetPixels(uint8_t *buffer, int xp, int yp, int wp, int hp, int stride)
{
	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(GetGraphics())->_cairo_context);
...
void GTKImage::GetPixels(uint8_t **buffer, int xp, int yp, int wp, int hp, int *stride)
{
	cairo_surface_t *cairo_surface = cairo_get_target(dynamic_cast<GTKGraphics *>(GetGraphics())->_cairo_context);
