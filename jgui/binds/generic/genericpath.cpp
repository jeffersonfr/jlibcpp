...
GenericPath::GenericPath(GenericGraphics *g):
	jcommon::Object()
{
	jcommon::Object::SetClassName("jgui::GenericPath");
...
void GenericPath::SetPattern(Image *image)
{
	if (image == NULL) {
		return;
	}
...
void GenericPath::SetPattern(int x0p, int y0p, int x1p, int y1p)
{
...
void GenericPath::SetPattern(int x0p, int y0p, int rad0, int x1p, int y1p, int rad1)
{
	cairo_t *cairo_context = _graphics->_cairo_context;
...
void GenericPath::MoveTo(int xp, int yp)
{
	struct jpoint_t t = _graphics->Translate();
...
void GenericPath::LineTo(int xp, int yp)
{
	struct jpoint_t t = _graphics->Translate();
...
void GenericPath::CurveTo(int x1p, int y1p, int x2p, int y2p, int x3p, int y3p)
{
	struct jpoint_t t = _graphics->Translate();
...
void GenericPath::ArcTo(int xcp, int ycp, int radius, double arc0, double arc1, bool negative)
{
	struct jpoint_t t = _graphics->Translate();
...
void GenericPath::TextTo(std::string text, int xp, int yp)
{
	GenericFont *font = dynamic_cast<GenericFont *>(_graphics->GetFont());
...
void GenericPath::Close()
{
	cairo_close_path(_graphics->_cairo_context);
}

void GenericPath::Stroke()
{
...
void GenericPath::Fill()
{
	cairo_fill(_graphics->_cairo_context);
}

void GenericPath::SetSource(Image *image)
{
	GenericGraphics *g = dynamic_cast<GenericGraphics *>(image->GetGraphics());
...
void GenericPath::SetMask(Image *image)
{
	GenericGraphics *g = dynamic_cast<GenericGraphics *>(image->GetGraphics());
...
void GenericPath::SetMatrix(double *matrix)
{
}

void GenericPath::GetMatrix(double **matrix)
{
}
