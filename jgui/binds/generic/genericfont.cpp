	...
	jgui::Font(name, attributes, size)
{
	...
	for (int i=0; i<256; i++) {
		...
		bounds = GenericFont::GetGlyphExtends(i);
...
void GenericFont::ApplyContext(void *ctx)
{
...
void * GenericFont::GetNativeFont()
{
	return _font;
}

std::string GenericFont::GetName()
{
	return _name;
}

int GenericFont::GetAscender()
{
	return _ascender;
}

int GenericFont::GetDescender()
{
	return abs(_descender);
}

int GenericFont::GetMaxAdvanceWidth()
{
	return _max_advance_width;
}

int GenericFont::GetMaxAdvanceHeight()
{
	return _max_advance_height;
}

int GenericFont::GetLeading()
{
	return _leading;
}

int GenericFont::GetStringWidth(std::string text)
{
...
jregion_t GenericFont::GetGlyphExtends(int symbol)
{
	cairo_glyph_t glyph;
	cairo_text_extents_t t;
...
bool GenericFont::CanDisplay(int ch)
{
	return true;
}

int GenericFont::GetCharWidth(char ch)
{
	return _widths[(int)ch];
}

const int * GenericFont::GetCharWidths()
{
	return (int *)_widths;
}

void GenericFont::Release()
{
}

void GenericFont::Restore()
{
}
