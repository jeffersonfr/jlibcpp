...
{
	jcommon::Object::SetClassName("jgui::SDLInputManager");
...
void SDLInputManager::SetKeyEventsEnabled(bool b)
{
	_is_key_enabled = b;
}

void SDLInputManager::SetMouseEventsEnabled(bool b)
{
	_is_mouse_enabled = b;
}

bool SDLInputManager::IsKeyEventsEnabled()
{
	return _is_key_enabled;
}

bool SDLInputManager::IsMouseEventsEnabled()
{
	return _is_mouse_enabled;
}

void SDLInputManager::SetClickDelay(int ms)
{
	if (ms > 0) {
		_click_delay = ms;
	}
...
int SDLInputManager::GetClickDelay()
{
	return _click_delay;
}

void SDLInputManager::PostEvent(KeyEvent *event)
{
	// SDL_PushEvent
}

void SDLInputManager::PostEvent(MouseEvent *event)
{
	// SDL_PushEvent
}

void SDLInputManager::RegisterKeyListener(KeyListener *listener) 
{
	jthread::AutoLock lock(&_mutex);
...
void SDLInputManager::RemoveKeyListener(KeyListener *listener) 
{
	jthread::AutoLock lock(&_mutex);
...
void SDLInputManager::DispatchEvent(jcommon::EventObject *event)
{
	jthread::AutoLock lock(&_mutex);
...
void SDLInputManager::RegisterMouseListener(MouseListener *listener) 
{
	jthread::AutoLock lock(&_mutex);
...
void SDLInputManager::RemoveMouseListener(MouseListener *listener) 
{
	jthread::AutoLock lock(&_mutex);
...
jkeyevent_symbol_t SDLInputManager::TranslateToSDLKeySymbol(SDL_Keysym symbol)
{
	switch (symbol.sym) {
		case SDLK_RETURN:
...
void SDLInputManager::ProcessInputEvent(SDL_Event event)
{
	jthread::AutoLock lock(&_mutex);
	...
	if (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) {
		...
		jkeyevent_symbol_t symbol = TranslateToSDLKeySymbol(event.key.keysym);
...
void SDLInputManager::Run()
{
	...
	while (_is_initialized == true) {
		while (SDL_PollEvent(&event)) {
			if (event.type == USER_SDL_EVENT_ENGINE_INIT) { // engine init
				SDLHandler *handler = (SDLHandler *)event.user.data1;

				handler->InternalInitEngine();

				puts("Handler 01");
			} else if (event.type == USER_SDL_EVENT_ENGINE_RELEASE) { // engine init
				SDLHandler *handler = (SDLHandler *)event.user.data1;

				handler->InternalRelease();

				puts("Handler 01");
			} else if (event.type == USER_SDL_EVENT_WINDOW_CREATE) { // window create
				Window *window = (Window *)event.user.data1;

				window->InternalInstanciateWindow();

				puts("Window 01");
			} else if (event.type == USER_SDL_EVENT_WINDOW_REPAINT) { // window repaint
				Window *window = (Window *)event.user.data1;
				Component *cmp = (Component *)event.user.data2;

				window->InternalRepaint(cmp);

				puts("Repaint Window 01");
			} else {
