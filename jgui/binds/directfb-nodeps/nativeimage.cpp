...
#define Native_FIXED_POINT	(1 << 16)
...
NativeImage::NativeImage(void *surface, jpixelformat_t pixelformat, int width, int height):
...
jsize_t NativeImage::GetImageSize(std::string img)
{
	jsize_t t;
...
Image * NativeImage::CreateImageStream(jio::InputStream *stream)
{
	if ((void *)stream == NULL) {
		return NULL;
	}
	...
	image = new NativeImage(NULL, JPF_ARGB, sdsc.width, sdsc.height);
...
Image * NativeImage::Flip(Image *img, jflip_flags_t mode)
{
	jsize_t size = img->GetSize();

	NativeImage *image = new NativeImage(NULL, img->GetPixelFormat(), size.width, size.height);
	...
	static const s32 x_mat[9] = {
		...
		0, Native_FIXED_POINT, 0,
		0, 0, Native_FIXED_POINT
	};

	static const s32 y_mat[9] = {
		Native_FIXED_POINT, 0, 0,
		0, -Native_FIXED_POINT, (size.height-1) << 16,
		0, 0, Native_FIXED_POINT
	};
...
Image * NativeImage::Rotate(Image *img, double radians, bool resize)
{
	...
	NativeImage *rotate = new NativeImage(NULL, ipixel, iw, ih);
	...
	static const s32 r_mat[9] = {
		(s32)(Native_FIXED_POINT*cos(angle)), (s32)(Native_FIXED_POINT*sin(angle)), (s32)(Native_FIXED_POINT*(dw/2)),
		(s32)(-Native_FIXED_POINT*sin(angle)), (s32)(Native_FIXED_POINT*cos(angle)), (s32)(Native_FIXED_POINT*(dh/2)),
		0, 0, Native_FIXED_POINT
	};
...
Image * NativeImage::Scale(Image *img, int width, int height)
{
	if (img == NULL) {
		return NULL;
	}

	NativeImage *image = new NativeImage(NULL, img->GetPixelFormat(), width, height);
...
Image * NativeImage::Crop(Image *img, int x, int y, int width, int height)
{
	if (img == NULL) {
		return NULL;
	}
...
Image * NativeImage::Blend(Image *img, double alpha)
{
	NativeImage *image = new NativeImage(NULL, img->GetPixelFormat(), img->GetWidth(), img->GetHeight());
...
Image * NativeImage::Colorize(Image *img, Color color)
{
	NativeImage *image = new NativeImage(NULL, img->GetPixelFormat(), img->GetWidth(), img->GetHeight());
...
void NativeImage::SetPixels(uint8_t *buffer, int xp, int yp, int wp, int hp, int stride)
{
	if (buffer == NULL) {
		return;
	}
...
void NativeImage::GetPixels(uint8_t **buffer, int xp, int yp, int wp, int hp, int *stride)
{
	IDirectFBSurface *surface = (IDirectFBSurface *)GetGraphics()->GetNativeSurface();
