...
void NativeHandler::Add(Font *font)
{
...
void NativeHandler::Remove(Font *font)
{
	jthread::AutoLock lock(&_mutex);
...
void NativeHandler::Add(Image *image)
{
	jthread::AutoLock lock(&_mutex);
...
void NativeHandler::Remove(Image *image)
{
	jthread::AutoLock lock(&_mutex);
...
IDirectFBDisplayLayer * NativeHandler::GetDisplayLayer()
{
	return _layer;
}

bool NativeHandler::CreateFont(std::string name, int size, IDirectFBFont **font)
{
...
void NativeHandler::InitEngine()
{
	...
	*/
	...
	// _layer->SetCooperativeLevel(_layer, (NativeDisplayLayerCooperativeLevel)(DLSCL_EXCLUSIVE));
