...
public:
	...
	CBitmap(jio::InputStream *stream) : m_BitmapData(0), m_BitmapSize(0) {
		Load(stream);
	}
	...
	bool Load(jio::InputStream *stream) {
		...
		stream->Read((char*) &m_BitmapFileHeader, BITMAP_FILEHEADER_SIZE);
		if (m_BitmapFileHeader.Signature != BITMAP_SIGNATURE) {
			return false;
		}

		stream->Read((char*) &m_BitmapHeader, sizeof(BITMAP_HEADER));
		
		/* Load Color Table */
		
		stream->Reset();
		stream->Skip(BITMAP_FILEHEADER_SIZE + m_BitmapHeader.HeaderSize);
		...
		stream->Read((char*) ColorTable, sizeof(BGRA) * m_BitmapHeader.ClrUsed);
		...
		stream->Reset();
		stream->Skip(m_BitmapFileHeader.BitsOffset);
		...
		if (m_BitmapHeader.Compression == 0) {
			for (unsigned int i = 0; i < GetHeight(); i++) {
				stream->Read((char*) Line, LineWidth);
			...
			while (stream->Available() > 0) {
				...
				stream->Read((char*) &ColorIndex, sizeof(uint8_t));
				...
				if (Count > 0) {
					...
					if (Flag == 0) {
						...
						char rx = 0;
						char ry = 0;
						stream->Read((char*) &rx, sizeof(char));
						stream->Read((char*) &ry, sizeof(char));
						x += rx;
						y += ry;
					} else {
						Count = Flag;
