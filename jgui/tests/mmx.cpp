...
int main(int argc, char *argv[])
{
  ...
  tifa = new jgui::BufferedImage("tifa.bmp");

  /*
  img_size = tifa->GetSize();

  filtered = new uint32_t[img_size.width*img_size.height];

  std::cout << "Processing zoom out ..." << std::flush;	

  {
    tifa->GetRGBArray(&pixels, 0, 0, img_size.width, img_size.height);

    size = img_size.width*img_size.height;

    for (int i=0; i<MAX_LOOP; i++) {
      zoom_out((uint8_t *)pixels, (uint8_t *)filtered, img_size.width, img_size.height, img_size.width/2, img_size.height/2, img_size.width);
    }

    memcpy(pixels, filtered, 3*img_size.width*img_size.height);
  ...
  {
    for (int i=0; i<MAX_LOOP; i++) {
      process_gray_mmx((uint8_t *)pixels, (uint8_t *)filtered, img_size.width, img_size.height);
    }
  }
  ...
  {
    for (int i=0; i<MAX_LOOP; i++) {
      process_brightness_mmx((uint8_t *)pixels, (uint8_t *)filtered, img_size.width, img_size.height, 3);
    }
  }
  ...
  {
    for (int i=0; i<MAX_LOOP; i++) {
      chroma_key_mmx((uint8_t *)pixels, img_size.width, img_size.height, chroma_pixels, chroma_width, chroma_height, filtered);
    }
  }
...

  {
    for (int i=0; i<MAX_LOOP; i++) {
      ...
      thresholding_adaptative(pixels, filtered, img_size.width, img_size.height);
    }

    // INFO:: detecta as bordas
    edge_detect(filtered, pixels, img_size.width, img_size.height);
    edge_follow(pixels, img_size.width, img_size.height, 64);
  }
  ...
  {
    for (int i=0; i<MAX_LOOP; i++) {
      histogram_equalization_global(pixels, filtered, img_size.width, img_size.height, 255);
    }
  }
  ...
  {
    for (int i=0; i<MAX_LOOP; i++) {
      histogram_equalization_local(pixels, filtered, img_size.width, img_size.height, 3);
    }
  }
  */
