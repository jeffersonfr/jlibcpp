 ...
 */
class TaskQueue : public virtual jcommon::Object {
...
class TimerThread {
    ...
    bool _is_running;
		/** \brief */
		bool _new_tasks_may_be_scheduled;
...
class Timer : public virtual jcommon::Object {
     ...
     * \brief Schedule the specifed timer task for execution at the specified time with the specified period, in microseconds.  
		 ...
		 * time, but not period.
     */
    void InternalSchedule(TimerTask *task, uint64_t next_execution_time, uint64_t delay, bool push_time);
