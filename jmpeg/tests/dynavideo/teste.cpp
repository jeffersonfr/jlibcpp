			...
			if (event->GetTID() == 0x3c) {
				...
				if (objectKind == 0x66696c00) { // 'f', 'i', 'l', '\0'
					...
					for (int i=0; i<eventNames_count; i++) {
						...
						if (name.find("nclEditingCommand") != std::string::npos) {
							_is_gingancl = true;
						}
				...
				while (descriptors_count < descriptors_length) {
					...
					if (descriptor_tag == 0x01) { // npt reference descriptor
						...
						if (_is_gingancl == false) {
							...
							printf(":: stream event descriptor:: event id:[%04x], event npt:[%lu], command tag:[%02x], final flag:[%d], sequence number:[%d, %d]\n", event_id, event_NPT, command_tag, final_flag, sequence_number, _dsmcc_sequence_number);
							if (final_flag == 0) {
