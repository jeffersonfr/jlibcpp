...
/**
 * \breif Encodes input (UINT4) into output (unsigned char). Assumes len is a multiple of 4.
 */
...
typedef struct {
	...
	char *hostname;
	unsigned char sdes_len[RTCP_SDES_MAX];
...
struct SDES_Item {
  	unsigned char type;                  /* type of item (Rtcp_Sdes_Type) */
...
typedef void (*Rtcp_Member_SDES_Func) (Rtp_Source *s, unsigned char type, char *data, unsigned char length);
...
class RTPSocket : public virtual jcommon::Object{
    ...
    public:
        ...
        *
        */
		void member_sdes(rtp_source *s, unsigned char type, char *data, unsigned char length);
