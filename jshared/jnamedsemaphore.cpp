	...
	jcommon::Object()
{
	...
	if (fd < 0) {
		throw jthread::SemaphoreException("Cannot open the semaphore resource file");
	}
	...
	jcommon::Object()
{
	...
	if (fd < 0) {
		throw jthread::SemaphoreException("Cannot create a semaphore resource file");
	}

	if (ftruncate(fd, sizeof(struct jnamedsemaphore_t)) != 0) {
		throw jthread::SemaphoreException("Semaphore resource file truncate error");
	}
...
void NamedSemaphore::Wait(uint64_t time_)
{
	...
	if (result == ETIMEDOUT) {
		throw jthread::SemaphoreTimeoutException("Semaphore wait timeout");
	} else if (result < 0) {
		throw jthread::SemaphoreException("Semaphore wait failed");
	}
#endif
}
...
void NamedSemaphore::NamedSemaphore::Notify()
{
	...
	if (_handler->count == 0) {
		if (pthread_cond_signal(&_handler->nonzero) != 0) {
			throw jthread::SemaphoreException("Condition notify error");
		}
	}
...
void NamedSemaphore::NamedSemaphore::NotifyAll()
{
	...
	if (pthread_cond_broadcast(&_handler->nonzero) != 0) {
		throw jthread::SemaphoreException("Condition notify all error");
	}
